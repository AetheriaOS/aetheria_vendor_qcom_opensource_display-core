  ...
  while (target_node != nullptr) {
    ...
    if (!strcmp(target_name, soc_name)) {
      ...
      while (panel_res_node != nullptr) {
        const int p_width = std::atoi(panel_res_node->Attribute("width"));
        const int p_height = std::atoi(panel_res_node->Attribute("height"));
        ...
        if (p_width == panel_width && p_height == panel_height) {
          ...
          while (scaling_factor_node != nullptr) {
            const float x_factor = std::atof(scaling_factor_node->Attribute("x"));
            const float y_factor = std::atof(scaling_factor_node->Attribute("y"));
